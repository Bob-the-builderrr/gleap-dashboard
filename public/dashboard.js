...
function initDatePickers() {
    ...
    updateDateInfo(startDate.toISOString().split('T')[0], endDate.toISOString().split('T')[0]);
}

// Update the timezone info with actual API dates
function updateDateInfo(startDate, endDate) {
    ...
    document.getElementById('timezoneInfo').innerHTML = `
        ...
        Selected Dates: ${startDate} to ${endDate}<br>
        API UTC Range: ${startUTC} to ${endUTC}
    `;
}
...
async function loadData() {
    ...
    try {
        ...
        console.log('Calling our API:', apiUrl);
