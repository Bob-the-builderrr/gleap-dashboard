...
export default async function handler(req, res) {
  try {
    ...
    const valid = rows.filter(r => r.ticket_id !== "-");
    const empty = rows.filter(r => r.ticket_id === "-");

    valid.sort((a, b) => Number(b.agent_open_ticket) - Number(a.agent_open_ticket));

    res.status(200).json([...valid, ...empty]);
