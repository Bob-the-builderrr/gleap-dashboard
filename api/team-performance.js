...
// Convert IST → UTC for given date/time
...
export default async function handler(req, res) {
  try {
    ...
    if (!response.ok) {
      throw new Error(`Gleap API responded with ${response.status}`);
    }
    ...
    res.status(200).json({ startDate: startUTC, endDate: endUTC, agents });
